...
const PICKUP_COLLECT_RANGE = 15; // same as PLAYER_RADIUS
...
class Game {
  constructor() {
    ...
    this.machineGunPickup = null; // { x, y, collected: false, collectedBy: null }
  }
  ...
  tryShoot(player) {
    const now = Date.now();
    const cooldown = player.shootCooldown || SHOOT_COOLDOWN_MS;
  ...
  checkPickupCollection() {
    ...
    for (const player of this.players.values()) {
      ...
      if (dist < PICKUP_COLLECT_RANGE) {
        ...
        player.shootCooldown = MACHINE_GUN_COOLDOWN_MS;
        break;
      }
    }
  }
